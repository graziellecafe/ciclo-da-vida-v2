...
class CicloDaVida {
    ...
    Genesis() {
        const grazielle = Pessoa_1.Pessoa.Nascimento('Grazielle', 0);
        this.CicloDaPessoa(grazielle);
    }
    CicloDaPessoa(pessoa) {
        let vivo = true;
        console.log('\n');
    ...
    ValidarFaseAdulta(idade) {
        if (idade >= 18 && idade <= 60) {
            return true;
        }
        return false;
    }
    Trabalhar(pessoa, diaDoMes, mes) {
    ...
    RealizaCompra(pessoa, valorDaCompra) {
    ...
    ValidarPagamentoEmprestimo(pessoa) {
    ...
    ValidarDiaDeDoacao(pessoa, diaDoMes) {
